...
class CustomerController {
    ...
    def create = {
        ...
        return [customerInstance: customerInstance, states: states, sponsors: Sponsor.findAllByInactive(false).sort {
            it.name
        }, statusList           : ['HAS_NOT_ORDERED': 'HAS NOT ORDERED', 'HAS_ORDERED': 'HAS ORDERED']]
    }

    def save = {
        println "CustomerController:save: ${params}"
        ...
        if (customerInstance.save(flush: true)) {
            ...
            render(view: "create", model: [customerInstance: customerInstance, states: states, sponsors: Sponsor.findAllByInactive(false).sort {
                it.name
            }])
        }
    }
    ...
    def show = {
        ...
        if (!customerInstance) {
            ...
            if (customerInstance.status == CustomerStatus.HAS_NOT_ORDERED) {
                ...
                return [customerInstance: customerInstance, products: products.findAll {
                    !it.parent
                ...
                return [customerInstance: customerInstance, products: products?.sort {
                    it?.product?.id
                }, customerCalls        : customerCalls,
                        rController     : params?.rController, rAction: params?.rAction, sort: params?.sort, offset: params?.offset, query: params?.query]
            }
    ...
    def edit = {
        ...
        if (!customerInstance) {
            ...
            return [customerInstance: customerInstance, products: products.sort {
                it.product?.id
            }, states               : states, broker: broker, statusList: statusList,
                    rController     : params?.rController, rAction: params?.rAction, sort: params?.sort, offset: params?.offset, query: params?.query]
        }
    }
    ...
    def add_order = {
        println "CustomerController:add_order: ${params}"
    ...
    def toggleDuplicate = {
        println "CustomerController:toggleDuplicate: ${params}"
        def customerInstance = Customer.get(params.id)
        def controller = params.rController
        def action = params.rAction
    ...
    def toggleDidNotReceive = {
        println "CustomerController:toggleDidNotReceive: ${params}"
    ...
    def toggleDidNotSample = {
        println "CustomerController:toggleDidNotSample"
    ...
    def adminAssessProduct = {
        println 'CustomerController:adminAssessProduct'
