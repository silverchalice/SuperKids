 ...
 <tr>
	 ...
	 <td  valign="top" colspan="2"><label>Please list any special requests for the manufacturers you checked:</label> <g:textField name="otherComments" value="${customerInstance?.otherComments}"  style="float:right"/></td>
			...
			<div style="clear:both;width:600px; border:0; text-align:center">
				...
				<g:actionSubmit class="save" action="update" value="${message(code: 'default.button.update.label', default: 'Save Changes')}" />
				</div>
					...
					<tbody>
					  <g:each in="${customerInstance.brokers}" var="broker">
						 <tr>
							...
							<td>
							  <g:link controller="product" action="brokerEditFromEdit" id="${broker?.id}" params="[rController:'home', rAction:'edit_profile']">Edit</g:link> | <g:link controller="product" action="brokerDeleteFromEdit" id="${broker?.id}" params="[rController:'home', rAction:'edit_profile']">Delete</g:link>
							</td>
						</tr>
					  </g:each>
					</tbody>
